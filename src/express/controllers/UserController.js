...
const Planet = require("../modals/PlanetModal")
...
class ProductController {
    ...
    findPlanet = async (req, res, next) => {
        const { name } = req.query;
        try {

            if (name) {
                const formattedName = name.charAt(0).toUpperCase() + name.slice(1).toLowerCase();
                const data = await Planet.findOne({ eng_name: formattedName });
    ...
    getAllPlanet = async (req, res, next) => {
